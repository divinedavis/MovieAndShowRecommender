...
const SLUG_CONFIG: Record<string, { title: string; description: string; runtimeGte?: string; runtimeLte?: string; sortBy: string }> = {
  'short-movies': {
    title: 'Best Short Movies Under 90 Minutes',
    description: 'Discover the best short movies under 90 minutes. Perfect for a quick watch when you do not have time for a long film.',
    runtimeLte: '90',
    sortBy: 'popularity.desc',
  },
  'long-movies': {
    title: 'Best Epic Movies Over 3 Hours',
    description: 'Explore the greatest epic movies over 3 hours long. These lengthy masterpieces are worth every minute of your time.',
    runtimeGte: '180',
    sortBy: 'popularity.desc',
  },
  'perfect-length': {
    title: 'Best Movies Between 90-120 Minutes',
    description: 'The highest-rated movies between 90 and 120 minutes. The perfect runtime for a satisfying movie experience.',
    runtimeGte: '90',
    runtimeLte: '120',
...
const SLUGS = Object.keys(SLUG_CONFIG);

async function fetchMoviesByDuration(slug: string) {
  ...
  if (config.runtimeGte) params.append('with_runtime.gte', config.runtimeGte);
  if (config.runtimeLte) params.append('with_runtime.lte', config.runtimeLte);
  ...
  return data.results.slice(0, 20).map((m: any) => ({
    ...
    year: m.release_date ? new Date(m.release_date).getFullYear() : 0, rating: m.vote_average || 0, description: m.overview,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const config = SLUG_CONFIG[slug];
  if (!config) return { title: 'Movie List' };
  return {
    title: config.title,
    description: config.description,
    keywords: [config.title.toLowerCase(), `${slug.replace(/-/g, ' ')}`, 'movie runtime', 'movies by length'],
    alternates: { canonical: `${BASE_URL}/lists/${slug}` },
    openGraph: { title: `${config.title} | UnitTap Movies`, description: config.description, type: 'website' },
  };
}

export default async function DurationListPage({ params }: Props) {
  const { slug } = await params;
  const config = SLUG_CONFIG[slug];
  if (!config) return <div>Not found</div>;
  const movies = await fetchMoviesByDuration(slug);
    ...
    <main className="min-h-screen bg-gray-50 text-gray-950 p-6 md:p-10">
      ...
      <header className="mb-20">
        ...
        <div className="flex gap-3 mt-6 flex-wrap">
          {SLUGS.map((s) => (
            <Link key={s} href={`/lists/${s}`} className={`border-4 border-black px-4 py-2 font-black text-sm uppercase ${s === slug ? 'bg-blue-600 text-white' : 'bg-white hover:bg-yellow-400'} transition`}>
              {SLUG_CONFIG[s].title.replace('Best ', '').split(' - ')[0]}
            </Link>
          ))}
        </div>
