...
export const metadata: Metadata = {
  title: "Best Book-to-Movie Adaptations - Novels That Became Great Films",
  description: "The best movies adapted from books and novels. From literary classics to bestselling page-turners, these adaptations brought beloved stories to life on screen.",
  openGraph: {
    title: "Best Book-to-Movie Adaptations - Novels That Became Great Films",
    description: "Top-rated movies based on novels and books.",
    url: "https://movies.unittap.com/lists/book-adaptations",
  },
  alternates: { canonical: "https://movies.unittap.com/lists/book-adaptations" },
};

export default async function BookAdaptationsPage() {
  // Keyword 818 = "based on novel or book"
  const [novelData, bookData] = await Promise.all([
  ...
  const allMovies: any[] = [];
  for (const m of [...novelData.results, ...bookData.results]) {
    if (!seen.has(m.id)) {
      seen.add(m.id);
      allMovies.push(m);
    }
  ...
  const jsonLd = {
    ...
    '@type': 'ItemList',
    name: 'Best Book-to-Movie Adaptations',
    description: 'Top-rated films adapted from novels and books.',
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-950 p-6 md:p-10">
      ...
      <header className="max-w-5xl mx-auto mb-12">
        ...
        <h1 className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter mb-4">Best Book-to-Movie Adaptations</h1>
        <p className="text-xs font-black tracking-widest text-blue-600 uppercase mb-4">Novels That Became Great Films</p>
        <p className="text-lg text-gray-600 max-w-3xl">From literary classics to modern bestsellers, some stories are so powerful they demand to be seen on screen. These are the highest-rated adaptations that honored their source material while creating something cinematic and unforgettable.</p>
      </header>
